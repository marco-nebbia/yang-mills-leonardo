...
NCOLORS=2
NTHREADS=1
compile_targets='yang_mills_local'
...
#./configure N_c=${NCOLORS} ST_dim=${STDIM} Num_threads=${NTHREADS} CC='gcc -Wno-deprecated-declarations' CFLAGS='-O3' ${flag_openmp} ${flag_theta}
...
#optimized configuration for Leonardo 
./configure N_c=${NCOLORS} ST_dim=${STDIM} Num_threads=${NTHREADS} Num_levels=${NLEVELS} CC=icc CFLAGS='-O3 -axCORE-AVX512 -mtune=sapphirerapids -ip -ipo' LIBS="-ldl -lz -lc" ${flag_openmp} ${flag_theta} ${flag_dirichlet}
